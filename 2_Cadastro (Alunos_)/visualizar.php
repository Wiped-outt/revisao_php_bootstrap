...
//  Verifica o ID não está vazio
if (!empty($id)) {
    // Query para selecionar informações de usuário e endereço associado
    ...
    // Verificar se a consulta foi bem-sucedida e se há registros
    if (($result_usuario) and ($result_usuario->rowCount() != 0)) {
        // Recupera os dados do usuário e endereço associado
        $row_usuario = $result_usuario->fetch(PDO::FETCH_ASSOC);
        $retorna = ['status' => true, 'dados' => $row_usuario];
    } else {
        // Se não houver registros, retorja uma mensgadem de erro
        $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro: Nenhum
